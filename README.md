...
Portable kit for Architecture Decision Records and live SSoT **before** every commit. Works with Cursor, Claude Code, Google Antigravity, and VS Code Copilot.

Clone (installable pack at repo root):

```powershell
gh repo clone javierborras/adrskill
```

https://github.com/javierborras/adrskill

Merge into another project from that project’s root:
...
```powershell
node .\adrskill\bin\cli.js create-adr --title "Auth via Google" --slug auth-google
node .\adrskill\bin\cli.js infer-northstar --goal "We build X for Y." --dry-run
```

Full Spanish instructions are above; this file is the GitHub landing page.
